...
describe("POST /items", () => {
  it("creates an item and returns it with a generated id", async () => {
    const res = await request(createApp()).post("/items").send({ name: "Widget", description: "A widget" });
  ...
  it("returns 400 when name is missing", async () => {
    const res = await request(createApp()).post("/items").send({ description: "no name" });
    expect(res.status).toBe(400);
    expect(typeof res.body.error).toBe("string");
  });

  it("returns 400 when name is an empty string", async () => {
    const res = await request(createApp()).post("/items").send({ name: "" });
    expect(res.status).toBe(400);
    expect(typeof res.body.error).toBe("string");
  });

  it("returns 400 when name is the wrong type", async () => {
    const res = await request(createApp()).post("/items").send({ name: 123 });
    expect(res.status).toBe(400);
    expect(typeof res.body.error).toBe("string");
  });
});
...
describe("GET /items", () => {
  ...
  it("does not leak items across separate createApp() calls", async () => {
    ...
    await request(appA).post("/items").send({ name: "Only in A" });
...
describe("GET /items/:id", () => {
  it("returns the item when it exists", async () => {
    const app = createApp();
    const created = await request(app).post("/items").send({ name: "Findable" });
...
describe("PATCH /items/:id", () => {
  it("updates all fields", async () => {
    const app = createApp();
    const created = await request(app).post("/items").send({ name: "Old", description: "old desc" });
  ...
  it("updates only the provided field, leaving others untouched", async () => {
    ...
    const res = await request(app).patch(`/items/${created.body.id}`).send({ description: "only desc" });
  ...
  it("returns 400 when name is set to an empty string", async () => {
    const app = createApp();
    const created = await request(app).post("/items").send({ name: "Old" });

    const res = await request(app).patch(`/items/${created.body.id}`).send({ name: "" });
    expect(res.status).toBe(400);
  });
...
describe("DELETE /items/:id", () => {
  it("deletes the item and subsequent get returns 404", async () => {
    const app = createApp();
    const created = await request(app).post("/items").send({ name: "Temporary" });
